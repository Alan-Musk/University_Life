...
def pingpong(n):
    ...
    """
    "*** YOUR CODE HERE ***"
    def count(index, derection,sum):
        if index == n:
            return sum
        if index % 8 == 0 or num_eights(index) != 0:
            return count(index+1,-derection,sum-derection)
        else:
            return count(index+1,derection,sum+derection)
    return count(1, 1,1)
...
def count_coins(total):
    ...
    def constrained_count(total,smallest_coin):
        ...
        with_coid=constrained_count(total-smallest_coin,smallest_coin)
        return without_coin+with_coid
